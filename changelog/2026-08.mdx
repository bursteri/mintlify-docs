...
<Update label="August 27, 2026">
  ...
  ## Install the PlainRouter CLI from PyPI

  **New** · Python package [`plainrouter==0.5.1`](https://pypi.org/project/plainrouter/) now includes the `plainrouter` command alongside the generated SDK. Install the isolated CLI with `pipx install plainrouter==0.5.1`, or run it as `python -m plainrouter` from an active environment. The PyPI and npm distributions expose the same nine-command interface and target signed API contract `0.5.0`. [Install the CLI](/cli/quickstart).
</Update>
