...
PlainRouter also passes the event ID to Meta. When you use the managed browser Pixel for the same event, PlainRouter uses one ID for the Pixel `eventID` and the server `event_id`. See [Meta `event_id` deduplication](https://plainrouter.com/library/meta-event-id-deduplication).
