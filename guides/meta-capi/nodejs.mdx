...
Pass the `event_id` returned by `sendPurchase`. An `accepted` delivery confirms Meta acceptance; `queued`, `sent`, or `retrying` remains pending. An empty list is not acceptance. For `failed:*` or `skipped:*`, inspect the [delivery explanation](/reference/statuses-and-terms#destination-delivery) before replaying anything. Handle `401` by checking credentials, `404` by checking ID, scope, and retention, and `422` by correcting the request fields.
