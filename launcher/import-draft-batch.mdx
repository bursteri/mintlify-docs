---
title: "Legacy Launcher: import a draft batch"
sidebarTitle: "Draft-file import"
description: "Import synced Drive assets from CSV or XLSX where legacy Launcher is enabled. Review validation errors and the closed execution boundary."
---
...
- A Write or Admin [workspace execution token](/mcp/workspace-tokens).
- Numeric source Meta ad set and ad IDs from the selected authorized account in that workspace.
...
`account_id` is the internal Plainrouter advertising-account ID, not the Meta external ID. Supply it when the workspace has multiple eligible active accounts; it can be omitted for the sole eligible account. Older account-bound keys still reject a different account.
...
For direct structured input instead of a file, use [`launcher.draft_batch`](/mcp/tools).
