---
title: "Legacy Launcher: connect Google Drive"
sidebarTitle: "Drive connection"
description: "Check legacy Launcher availability, verify ownership of a Google Drive folder, and troubleshoot read-only inventory sync. Disabled by default."
---

<Warning>
  Legacy Launcher is disabled by default. This guide applies only where Plainrouter has explicitly enabled it. While disabled, Drive connection and file-import routes return `404`, inventory sync does not run, and `launcher.*` tools are absent from MCP discovery. A workspace token cannot enable the feature.
</Warning>
...
<Info>
  When enabled, Launcher inventories Drive media metadata only. It does not download media bytes, upload media to Meta, create ads, or activate spend.
</Info>
...
<Steps>
  <Step title="Open workspace settings">
    Open the workspace settings in Plainrouter. Find **Launcher Drive access**.
  </Step>
