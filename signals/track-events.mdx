...
While consent is pending or denied, PlainRouter does not accept browser measurement events. It does not write or send visitor, user, click, IP, or user-agent identifiers from those events.

Granting consent creates or refreshes the first-party identifiers used for session joins and eligible destination delivery. Denying consent removes PlainRouter's visitor and click identifiers from cookies and local storage. Send the latest state whenever the visitor's choice changes.
