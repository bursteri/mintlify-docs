...
description: "Use the Signals v2 browser API to manage consent, identify visitors, and record conversion events."
---

The Signals v2 API is available as `window.signalq` after you install the generated pixel. Calls made before the asynchronous script loads remain in the queue.
...
```js
await signalq("consent", "granted");

// If the visitor later withdraws consent:
await signalq("consent", "denied");
```

Supported values are `pending`, `granted`, and `denied`.

<Warning>
  Do not infer consent. Keep the state `pending` until your consent-management platform has an explicit choice.
</Warning>

While consent is pending or denied, PlainRouter records anonymous events. They exclude visitor IDs, user data, click IDs, and URL query strings. At the collection edge, PlainRouter also removes the client IP and user agent before it queues the event. It can retain coarse country and collection-hostname context for measurement. PlainRouter does not send these events to connected advertising destinations.

Granting consent creates or refreshes the first-party identifiers used for session joins and eligible destination delivery. Denying consent removes PlainRouter's visitor and click identifiers from cookies and local storage.
