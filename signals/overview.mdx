...
Retention changes must pass the server-side retention policy. MCP does not expose a retention setter. The former retention-instruction history and export are no longer available; the retention policy still applies.
