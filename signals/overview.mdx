...
When you provision Signals, choose 30 or 90 days for visitor identity retention. The default is 90 days. The selection applies to the workspace's retained visitor identity state; it does not widen the fixed retention limits of every ledger table.

PlainRouter records the current retention instruction with its effective time, controller identity, selected value, selecting user, and optional justification and privacy-notice version. A retention change must pass the same server-side policy used by the dashboard and API. MCP does not expose a retention setter.
