...
Install [`plainrouter==0.5.1`](https://pypi.org/project/plainrouter/), then create a client with the Signals workspace secret:
...
Install [`plainrouter-sdk 0.1.0`](https://rubygems.org/gems/plainrouter-sdk), then create a client with the Signals workspace secret:
...
| TypeScript | `@plainrouter/sdk` | `0.5.0` | Signals workspace secret passed through `signalTrackerSecret` |
| Python | `plainrouter` | `0.5.1` | Signals workspace secret passed to `create_client` |
| Ruby | `plainrouter-sdk` | `0.1.0` | Signals workspace secret passed to `PlainRouter::Client.new` |
| Go | `github.com/plainrouter/sdk-go` | `v0.5.2` | Signals workspace secret attached to the request context |
| CLI | `plainrouter`, `@plainrouter/cli`, or `plainrouter/tap/plainrouter` | PyPI `0.5.1`; npm `0.5.0` | Stored compatibility-labeled tracker token or `PLAINROUTER_TOKEN` |
