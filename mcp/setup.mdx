...
Connect Claude Code to Plainrouter MCP, verify a synthetic response, then configure a workspace execution token for your own Meta ad account. A successful account-scoped read returns the intended workspace and selected account. For installation before connecting Meta, use the workspace-only read below.
...
| `https://plainrouter.com/mcp` | Inspect workspace Signals, read an approved account, and submit governed proposals. | Workspace execution token for tool calls and account data |
...
- An active Meta ad account connection for account-scoped reads and creative workflows. Workspace installation reads do not require one.
- An MCP-compatible client that can send a fixed bearer token to a remote HTTP server.
- Current team membership with permission for the key tier you need. [Check key permissions](/mcp/workspace-tokens#issue-a-token).
- A clear choice of the advertising account for account-scoped workflows.
...
<Note>
  ...
  account-data reads remain workspace-authorized and return HTTP `401` without a valid
...
<Steps>
  <Step title="Issue the token">
    In Plainrouter, switch to the intended workspace and open **Settings → Workspace keys**. Enter a name, choose Read or Write and an expiry, then select **Create key**.
  </Step>
  ...
  <Step title="Confirm the selected account">
    For account-scoped work, call `get_account_state` first and supply `account_id` when multiple eligible accounts exist. Confirm the returned workspace and account. For installation without Meta, use the workspace read below.
  </Step>
</Steps>

See [Workspace tokens](/mcp/workspace-tokens) for tier selection, account selection, replacement, and revocation.
...
Check `connection.health` as well as the selected account. It matches the provider status shown in Connections for this workspace; raw `connection.status: "active"` can still accompany `attention_needed`. Follow the [health status reference](/reference/statuses-and-terms#provider-connection-health).

For the returned fields and their meaning, see the [MCP tool reference](/mcp/tools).

### Inspect installation before connecting Meta

With a production Read key configured, ask the agent:

```text
Call get_install_instructions with no arguments. Confirm the workspace,
show its managed-hostname setup status and first-arrival state, then stop.
Do not change DNS or install anything.
```

The tool returns existing managed-hostname material. A null snippet means setup is not ready; same-origin forwarding is unsupported by this tool. After deploying the generated snippet yourself, repeat the read to check `first_arrival`. This is separate from the synthetic ingestion diagnostic.

For reporting, call `get_arrivals_comparison` with `{"days": 7}`. Check its workspace, period, status, and unavailable values before comparing arrivals and clicks. Neither of these tools requires `account_id`, and neither is included in the sandbox. See the [installation and arrivals tool reference](/mcp/tools#get_install_instructions).

### Use a built-in Signals prompt

On the production connection, open your client's prompt selector and choose **Install the count on my site**, **Compare claimed clicks with counted arrivals**, or **Is my count healthy**. The [prompt reference](/mcp/tools#built-in-signals-prompts) lists their names, intended tool calls, and limits.

Listing prompts needs no credential, but retrieving one requires your workspace key. Loading a prompt returns instructions; it does not run a tool or complete setup. Let the client call the referenced tool with the same workspace authorization and check the actual result. If your client has no prompt selector, use the direct tool examples above. The sandbox does not provide these prompts.
...
Check `account_id` and the key's workspace. New workspace keys can select an active account owned by that workspace; an older account-bound key rejects a different account. Follow [account selection](/mcp/workspace-tokens#how-does-account-selection-work).
