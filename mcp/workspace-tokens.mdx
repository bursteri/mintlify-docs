---
title: "Manage workspace tokens and grants"
sidebarTitle: "Workspace tokens"
description: "Issue account-bound MCP tokens, inspect workspace grant permissions, and verify token rotation or grant revocation in Plainrouter."
---
...
See [Authentication and clients](/api/authentication) for the canonical
...
<Steps>
  ...
  <Step title="Open the token page">
    Open **Tokens** in the Plainrouter dashboard and choose **Issue token**.
  </Step>
...
Sign in as the workspace owner and open **Workspace settings → Workspace grants**. Each grant shows its principal, tier, **Live**, **Expired**, or **Revoked** status, bound token name, and expiry. Expand **Exact operations** to inspect what it authorizes. The **Tokens** page shows token status and access tier; inspect the linked grant in **Workspace grants**.
...
Open **Tokens** in the dashboard to review token names, accounts, tiers, expiry, and last use. Enable **Show revoked** to include revoked records.
