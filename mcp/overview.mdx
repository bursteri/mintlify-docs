...
<CardGroup cols={2}>
  <Card title="Set up your connection" icon="plug" href="/actions/connect-agent">
    Configure Claude Code, try the sandbox, and verify the bound Meta ad account.
  </Card>
  <Card title="Explore the tools" icon="wrench" href="/reference/mcp-tools">
...
<Steps>
  <Step title="Connect a client">
    Use the sandbox without an account, or configure the production endpoint with a workspace execution token. Follow [Setup](/actions/connect-agent).
  </Step>
  ...
  <Step title="Choose a workflow">
    Start with a read. Use [Signals diagnostics](/reference/mcp-tools#get_signal_health), or follow the [creative proposal walkthrough](/actions/creative-workflow) when you are ready to request a change.
  </Step>
</Steps>
...
The [setup guide](/actions/connect-agent#configure-claude-code-for-your-meta-ad-account) includes Claude Code configuration. Other clients need remote HTTP MCP support and a way to send a fixed bearer credential. A client that requires OAuth sign-in for every remote server cannot use Plainrouter's production token flow as documented here.

## Access and approval

A workspace owner issues the token for the intended Meta ad account. Start with the **Read** tier for account and creative-library inspection; proposal-producing tools require **Write** or **Admin**. [Choose a token tier](/actions/workspace-tokens#access-tiers).
...
Policy checks, human approval, and provider verification govern supported creative execution. A proposal or approval is not **Landed**. Review the [tool reference](/reference/mcp-tools) for exact permissions and the [proposal review guide](/actions/review-proposals) for execution states.

## Continue with your task

- [Try the sandbox](/actions/connect-agent#start-in-test-mode) without accessing production data.
- [Connect your Meta ad account](/actions/connect-agent#connect-with-a-workspace-token) and verify the first read.
- [Create a paused creative variant](/actions/creative-workflow) with an explicit approval step.
- [Troubleshoot a connection](/actions/connect-agent#troubleshoot) or [rotate and revoke access](/actions/workspace-tokens#rotate-or-revoke-a-token).
