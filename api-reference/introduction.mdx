...
<CardGroup cols={2}>
  ...
  <Card title="Authenticate production requests" icon="key-round" href="/auth#how-do-i-authenticate-the-rest-api">
...
`accepted: true` confirms synthetic validation. It does not test your production credential, store an event, or verify Meta delivery. Continue with the [sandbox quickstart](/sandbox) to test bearer authentication, check allowed fields, or troubleshoot sandbox keys.
...
1. [Set up Signals](/quickstart), then configure your server with its [Signals workspace secret](/auth#how-do-i-authenticate-the-rest-api). Keep credentials out of browser code, prompts, and source control.
...
3. For advertising delivery, [connect the intended Meta dataset](/signals/connect-meta). Send an actual conversion only with the recorded consent and original event data required by the [conversion guide](/reference/conversion-api#submit-an-event).
4. Preserve the event ID on retries and [inspect the delivery trace](/reference/conversion-api#how-do-i-confirm-meta-accepted-the-event). HTTP `202` is ingestion acceptance; destination acceptance is a separate result.
...
See [conversion errors](/reference/conversion-api#handle-errors), [consent warnings](/reference/conversion-api#why-is-the-consent-capture-time-invalid), and [sandbox rate limits](/sandbox#stay-within-the-rate-limits) for the applicable details. Prefer an [official SDK](/sdk/overview) when you want typed operations in application code.
