...
<Warning>
  The CLI is a `0.x` preview and is not yet published to npm. Prepare your workflow now, but run the install command only after PlainRouter announces the first package release.
</Warning>
