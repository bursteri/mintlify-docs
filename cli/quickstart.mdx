...
The current npm release is `0.3.1` and uses `@plainrouter/sdk` `0.3.1`. Review release notes before upgrading while the CLI remains in the `0.x` series.

<Note>
  CLI `0.3.1` can create events with the current consent contract. It does not expose an ingestion-verification command; use the TypeScript or Python SDK, direct REST, or MCP for that operation.
</Note>
