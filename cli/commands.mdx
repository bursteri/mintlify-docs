...
PlainRouter CLI exposes nine commands through the PyPI `plainrouter` package `0.5.1`, npm `@plainrouter/cli` package `0.5.0`, and the official Homebrew tap. All distributions target signed API contract `0.5.0`. The contract also includes ingestion verification, which is available through the SDKs, direct REST, or MCP rather than a CLI command.
