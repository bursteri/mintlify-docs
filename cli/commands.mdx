...
Use a stable event ID when creating an event so retries remain idempotent. A `duplicate: true` receipt means the original event exists, not that Meta accepted it. Inspect `events get` before considering a delivery replay. Use the [sandbox](/sandbox) for synthetic validation without production writes.
