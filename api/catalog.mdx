...
3. Read [Authentication and clients](/api/authentication) before sending
   any bearer credential.
4. Confirm the key-scoped workspace and selected Meta ad account before using MCP.
...
index](/api/resources). For request and response schemas, use the
[Conversion API reference](/api/conversions).
