---
title: "Plainrouter authentication: API secrets and MCP tokens"
...
Sign in as the workspace owner, select the intended workspace, and open **Settings → API secret**. Complete Signals setup first if the page redirects you to setup. This secret authenticates REST, SDK, and CLI calls; [MCP tokens](/mcp/workspace-tokens) have their own **Tokens** page.
...
2. Update each integration and verify a read request, such as the event-list request above, with the replacement. Your current secret continues working while **Replacement pending** is displayed.
...
A workspace owner issues the token from **Tokens** in the dashboard for one workspace,
