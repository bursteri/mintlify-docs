...
| Manage accessible workspaces and issue workspace keys | Management API | Management key | Limited by its owner's current access and role; not an MCP credential |
...
A workspace owner creates a key in **Settings → Workspace keys** for one workspace,
a Read or Write tier, and a 30, 90, or 365 day expiry.
...
Management keys authenticate management operations under the owner's current access. Give agents a workspace key for MCP instead. A Management key is not the OAuth management credential described below and cannot authenticate the agent-context route, Signals Conversion API, or human approval pages.
