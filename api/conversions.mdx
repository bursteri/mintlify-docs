...
see [Authentication and clients](/api/authentication).

New to the API? Start with the [API introduction](/api/introduction) for the base URL, a synthetic first request, and endpoint navigation. This guide covers production conversion handling and recovery.

## Before you begin

You need an active Signals workspace, its [server-side secret](/api/authentication#how-do-i-authenticate-the-rest-api), and a recorded consent decision for the conversion. To deliver the event to Meta, [connect the intended dataset](/signals/connect-meta) first.

Start with the [identity-free ingestion diagnostic](/signals/verify-ingestion) if you only need to check authentication. Use the [sandbox](/api/sandbox) for synthetic payload validation or a [controlled destination test](/signals/connect-meta#test-without-sending-a-real-purchase) to check Meta delivery. The conversion request below can create a real event and destination delivery.
...
| [Plainrouter CLI](/sdk/cli/quickstart) | You want to test an operation or run an operational script from a terminal. |
...
The [sandbox operations](/api/sandbox) have separate authentication rules.
...
The Conversion API does not accept an MCP OAuth token or a [workspace token](/mcp/workspace-tokens). Workspace execution tokens authorize scoped agent tools. OAuth management credentials are limited to management discovery and cannot authenticate MCP tool calls. Neither authenticates the Signals API.
...
For a server implementation that retains the original consent timestamp, use the [Plainrouter SDK purchase recipe](/sdk/recipes/meta-conversions). The [published OpenAPI contract](https://plainrouter.com/openapi.json) defines the acknowledgement fields; older generated SDK models may not expose newly added fields.
...
- You are using the [Signals workspace secret](/api/authentication#how-do-i-authenticate-the-rest-api) for the event and destination.
