...
<Warning>
  A **Sandbox key** is not a Signals **Server secret** or a **Workspace key**. It authenticates only the synthetic sandbox, has no workspace or Meta ad-account binding, and grants no production access.
</Warning>
...
Registration is self-serve. Review the current EUR spend bands and free arrival counting in [Pricing and billing](/billing/overview). A current workspace owner or team member can issue role-capped [Workspace keys](/mcp/workspace-tokens) from the dashboard.

Keep the production credential types separate. A Server secret authenticates the Signals Conversion API for one workspace. A Workspace key authenticates MCP and authorized workspace routes for one workspace and an eligible selected advertising account. Older account-bound keys retain their binding. Neither credential is a sandbox key.
