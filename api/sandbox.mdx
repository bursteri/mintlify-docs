...
Keep the production credential types separate. A Signals workspace secret authenticates the Signals Conversion API for one workspace. A workspace execution token authenticates MCP and authorized workspace routes for one workspace and one Meta ad account. Neither credential is a sandbox key.
