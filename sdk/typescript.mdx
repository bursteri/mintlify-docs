...
The current npm release is `0.3.1` and targets signed API contract `0.3.1`. Review release notes before upgrading while the SDK remains in the `0.x` series.

<Note>
  Version `0.3.1` includes the required event consent fields and `verifySignalIngestion`.
</Note>
