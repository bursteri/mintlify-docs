...
<Warning>
  SDK `0.2.0` targets API contract `0.2.0`. The signed REST contract is now `0.3.1`: it requires `consent_basis` for event creation and adds `verifySignalIngestion`. Use direct REST for those operations until an updated SDK is released.
</Warning>
