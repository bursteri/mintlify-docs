...
The client sends the Signals workspace secret as a bearer token. The `signalTrackerSecret` property keeps its compatibility name in `0.5.0`. The client does not read credentials from the environment automatically.
