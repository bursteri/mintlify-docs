...
- A [Signals workspace secret](/api/authentication#how-do-i-authenticate-the-rest-api) stored only on your server.
...
Use [Signal health and performance](/signals/health-and-performance) or retrieve the event with `getEvent` to inspect its delivery state. During setup, you can also use Meta Test Events mode through the destination operations in the [Conversion API reference](/api/conversions).
...
- [Plainrouter Conversion API reference](/api/conversions)
