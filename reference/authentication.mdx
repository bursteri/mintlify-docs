...
description: "Choose the correct PlainRouter credential and authenticate REST, TypeScript, Python, CLI, MCP, or management-discovery requests."
---
...
REST, TypeScript, Python, and the CLI are four interfaces to the same Conversion
...
Install `@plainrouter/sdk@0.3.1`, then configure the generated client explicitly:
...
Install `plainrouter==0.3.1`, then create a client with the Signal tracker secret:
...
| --- | --- | --- | --- |
| REST | `https://plainrouter.com/api/v1` | Signed OpenAPI `0.3.1` | Signal tracker secret |
| TypeScript | `@plainrouter/sdk` | `0.3.1` | Signal tracker secret passed to `configurePlainrouter` |
| Python | `plainrouter` | `0.3.1` | Signal tracker secret passed to `create_client` |
| CLI | `@plainrouter/cli` | `0.3.1` | Stored tracker token or `PLAINROUTER_TOKEN` |
| MCP | `https://plainrouter.com/mcp` | Server `0.3.0` | Workspace execution token |

The REST contract and both SDKs expose ten Conversion API operations. CLI
`0.3.1` exposes nine commands and does not include an ingestion-verification
command. Use REST, either SDK, or MCP for ingestion verification.
