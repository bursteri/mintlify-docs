...
| State | Recovery |
| --- | --- |
| `failed:auth` | [Reconnect Meta](/signals/connect-meta#refresh-available-assets), then [replay the eligible delivery](/reference/conversion-api#how-do-i-replay-a-failed-delivery). |
| `failed:permanent` or `failed:unknown` | Read the [trace explanation](/reference/conversion-api#read-a-failed-delivery-trace). Correct the cause before considering replay; replay preserves the existing payload. |
| `expired` | Check payload availability and original event time. [Replay limits](/reference/conversion-api#how-do-i-replay-a-failed-delivery) still apply; do not reset timestamps. |
