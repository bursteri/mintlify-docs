---
title: "Plainrouter delivery statuses and workflow terms"
sidebarTitle: "Statuses and terms"
description: "Diagnose Meta delivery statuses and interpret Signals health, Actions proposals, execution, verification, and landing terms in Plainrouter."
---

Use the exact status from an event trace or workflow record to choose the next step. Ingestion receipt, Meta acceptance, and verified Actions execution describe different outcomes.

## Provider connection health

`get_account_state` returns `connection.health` using the same workspace-specific status as **Connections**. The existing `connection.status` is the raw stored connection state; do not substitute it for health.

| Value | Meaning | Next step |
| --- | --- | --- |
| `not_connected` | No usable bound connection is configured for this workspace, or the stored connection is inactive. | Open Connections and check the selected platform account. |
| `revoked` | The connection is marked revoked or provider-token revocation completed. | Reconnect the intended platform account if access should resume. |
| `attention_needed` | Reconnection is required, credentials are missing, the latest stored read failed, or revocation is unfinished. | Inspect Connections and its last-read state; resolve the reported issue before relying on platform data. |
| `connected` | The active connection has stored credentials and a bound account, without a failed latest read or pending revocation. | Check data freshness and destination delivery separately. |

These values describe stored connection evidence. `connected` can occur before any successful platform read; it does not certify credentials with a new provider request. For Meta, use [connection setup and permissions](/signals/connect-meta) and [delivery checks](/signals/connect-meta#how-do-i-verify-meta-delivery). Provider health is also separate from the Signals diagnostic below.

## Signal health

These labels come from the account-scoped `get_signal_health` diagnostic. The Signals overview presents collection status and arrival metrics separately.

| API value | UI label | Meaning |
| --- | --- | --- |
| `setup_required` | Setup required | Signal collection or hostname activation is incomplete. |
| `awaiting_data` | Awaiting data | Setup is active, but no retained arrival evidence exists. |
| `quiet` | Quiet | Retained arrival evidence exists, but none falls in the selected window or today. |
...
### Workspace Signals state

| Value | Meaning |
| --- | --- |
| `pending` | Setup has started but is not fully active. |
| `active` | The workspace can receive Signals events. |
| `inactive` | Signals is not active for the workspace. |

### Collection hostname

`removed_at_cloudflare` means Cloudflare removed the managed hostname. Automatic refresh stops; use **Re-provision hostname** to recover. The removed record remains as history.
...
| `removed_at_cloudflare` | The managed hostname was removed at Cloudflare and requires re-provisioning. |
...
| `expired` | Terminal | The event exceeded delivery freshness or its retained payload is unavailable. |
...
### What should I do after a delivery fails?

| State | Recovery |
| --- | --- |
| `failed:auth` | [Reconnect Meta](/signals/connect-meta#refresh-available-assets), then [replay the eligible delivery](/api/conversions#how-do-i-replay-a-failed-delivery). |
| `failed:permanent` or `failed:unknown` | Read the [trace explanation](/api/conversions#read-a-failed-delivery-trace). Correct the cause before considering replay; replay preserves the existing payload. |
| `expired` | Check payload availability and original event time. [Replay limits](/api/conversions#how-do-i-replay-a-failed-delivery) still apply; do not reset timestamps. |
| `skipped:consent` | [Fix consent handling](/signals/track-events#set-consent) for future eligible events. Skipped deliveries cannot be replayed. |
| `skipped:no_destination` | [Connect the intended destination](/signals/connect-meta#connect-the-destination) for future events. This does not replay skipped history. |
| `skipped:duplicate` | Inspect the original delivery; do not send another purchase with a new ID. |
...
A receipt marked `irreversible` does not establish restoration. A batch with `rollback_incomplete` needs investigation even if other actions were restored. Read the exact action and receipt outcomes before retrying or claiming that a provider change was undone.
...
<AccordionGroup>
  ...
  <Accordion title="Landed">
    ...
    The **Inbox** display group named **Landed** also includes `approved_without_execution` recommendations. Check the action status and receipt; that group alone does not prove provider execution. See [Inbox groups](/actions/review-proposals#read-the-inbox-groups).
  </Accordion>
