...
The signed REST contract, TypeScript SDK, Go SDK, and npm CLI are published at version `0.5.0`. Python package `0.5.1` targets the same signed contract and adds the PyPI CLI distribution. See [Client releases](#client-releases) for the CLI's command boundary.
...
The TypeScript package [`@plainrouter/sdk@0.5.0`](https://www.npmjs.com/package/@plainrouter/sdk), Python package [`plainrouter==0.5.1`](https://pypi.org/project/plainrouter/), and Go module [`github.com/plainrouter/sdk-go@v0.5.0`](https://pkg.go.dev/github.com/plainrouter/sdk-go) are generated from the signed `0.5.0` ten-operation contract. All three expose ingestion verification and the required event consent fields.
