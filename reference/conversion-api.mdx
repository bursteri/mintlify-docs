...
The signed REST contract, TypeScript SDK, Go SDK, and npm CLI are published at version `0.5.0`. Python package `0.5.1` and Ruby SDK `0.1.0` target the same signed contract. The Python package also adds the PyPI CLI distribution. See [Client releases](#client-releases) for package links and the CLI's command boundary.
...
The TypeScript package [`@plainrouter/sdk@0.5.0`](https://www.npmjs.com/package/@plainrouter/sdk), Python package [`plainrouter==0.5.1`](https://pypi.org/project/plainrouter/), Ruby gem [`plainrouter-sdk 0.1.0`](https://rubygems.org/gems/plainrouter-sdk), and Go module [`github.com/plainrouter/sdk-go@v0.5.0`](https://pkg.go.dev/github.com/plainrouter/sdk-go) are generated from the signed Conversion API contract `0.5.0`. All four expose ingestion verification and the required event consent fields.
