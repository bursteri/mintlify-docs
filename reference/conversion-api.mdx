...
see [Authentication and clients](/reference/authentication).
...
The signed REST contract, TypeScript SDK, Python SDK, and CLI are published at version `0.3.1`. See [Client releases](#client-releases) for the CLI's command boundary.
...
The TypeScript package `@plainrouter/sdk@0.3.1` and Python package `plainrouter==0.3.1` are generated from the signed ten-operation contract. Both expose ingestion verification and the required event consent fields.

The CLI `@plainrouter/cli@0.3.1` uses the current TypeScript SDK and can send the consent-aware event shape. It exposes nine terminal commands but does not add a command for `verifySignalIngestion`; use either SDK, direct REST, or MCP for that operation.
