...
The published `@plainrouter/sdk` and `@plainrouter/cli` packages are at version `0.2.0` and target PlainRouter API contract `0.2.0`. Review release notes before upgrading while the packages remain in the `0.x` series.
...
The document uses OpenAPI `3.1.0`, publishes API version `0.2.0`, and includes `x-signed: true`. PlainRouter serves the reviewed bytes with an ETag so automated tooling can detect a new contract without downloading an unchanged document.
