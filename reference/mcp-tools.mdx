...
description: "Reference for the 13 production tools and four synthetic test-mode tools exposed by PlainRouter MCP."
---

PlainRouter MCP server version `0.3.2` exposes 13 production tools. Account-scoped tools use the advertising account selected during workspace-token issuance.
...
These tools accept one `batch_id`: the 26-character ULID returned by `launcher.draft_batch` or the file-import adapter. PlainRouter resolves it only inside the workspace and ad account bound to the token. A caller cannot inspect another account by guessing an ID.
