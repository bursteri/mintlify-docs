---
title: "MCP tools"
description: "Reference for the seven account-scoped tools exposed by the PlainRouter MCP server."
---

PlainRouter MCP server version `0.3.0` exposes exactly seven tools. All use the advertising account selected by the person during OAuth authorization or workspace-token issuance.
...
Signals read tools require an active principal with `ad-account.read`. `propose-actions` requires `actions.propose`. Creative-library reads require `creative.read`. Creative proposal tools require both `creative.write` and `actions.propose`.
