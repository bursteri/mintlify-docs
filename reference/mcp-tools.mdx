...
PlainRouter MCP server version `0.3.0` exposes exactly nine tools. All use the advertising account selected during workspace-token issuance.
...
Clients authenticate with an owner-issued [workspace execution token](/actions/workspace-tokens). A **Read** token derives the read scopes. **Write** and **Admin** tokens also derive the proposal scopes. OAuth management credentials cannot authenticate to the MCP server.

`launcher.draft_batch` and the file-import adapter require the Write or Admin tier. The other tools use their derived scopes as listed above.
