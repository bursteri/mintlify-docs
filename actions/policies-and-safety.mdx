...
| Legacy MCP `adjust_budget` and `set_status` execution | Disabled. |
...
Next, [review a proposal](/actions/review-proposals) against its frozen mode and current constraints. Find policy changes and execution-time refusals in the [audit log](/actions/audit-log).
