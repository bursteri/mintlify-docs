...
Use [proposal history](/actions/review-proposals) for the human decision and [action policy](/actions/policies-and-safety) to interpret blocks. If access must end, [revoke the workspace token](/mcp/workspace-tokens). Drive ownership events apply only to [legacy Launcher](/launcher/connect-drive) where enabled.
