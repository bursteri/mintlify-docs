...
description: "Evaluate proposal evidence, policy checks, execution consequences, and verification state before approving or rejecting a governed Action."
---

Open **Approval queue** from the Plainrouter sidebar to review agent proposals for the current workspace. For a complete example from library selection to a decision, follow [Create a paused Meta ad variant](/actions/creative-workflow#example-propose-a-paused-creative-variant).
...
Confirm every target and parameter. For a creative duplication proposal, `target_entity.id` is the source ad; the new Meta ad ID appears in the execution receipt only after a provider write. The approval control states the consequence:
...
If a proposal is blocked, inspect the stated [policy constraint](/actions/policies-and-safety) before resubmitting. For pending verification or recovery, follow its batch in the [audit log](/actions/audit-log). To build a supported proposal, start with the [creative workflow](/actions/creative-workflow).
