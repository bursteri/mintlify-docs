...
<Steps>
  <Step title="A person authorizes the workspace">
    A workspace owner creates a Read or Write workspace key. Account-scoped tools select an eligible active account owned by that workspace; older bound keys keep their account restriction. Confirm the selected Meta ad account before proposing a change.
  </Step>
